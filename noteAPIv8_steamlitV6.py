...
def get_db_info():
    # V6専用の環境変数を優先的に読み込む
    db_url = os.getenv("DATABASE_URL_V6")
    if not db_url:
    ...
    if db_type == "postgres":
        ...
        if user_id:
            with conn.cursor() as cur:
                # Row Level Security (RLS) のためのユーザーID申告
                cur.execute("SET app.current_user_id = %s", (user_id,))
        return conn
    else:
        return sqlite3.connect(db_target)

def get_current_user_id(note_email=None):
    """
    ユーザーIDを一貫して生成する (noteDashboardWorkspaceと共通)
    """
    email = st.session_state.get("app_user_email")
    if not email:
        email = note_email
    if not email:
        return "guest"
...
def get_connection_wrapped():
    """
    セッションからIDを自動取得して接続するラッパー (V6追加)
    """
    uid = get_current_user_id()
    return get_connection(user_id=uid)

def init_db_schema():
    db_type, _ = get_db_info()
    try:
        # スキーマ作成時は管理者権限またはIDなしで接続
...
def admin_delete_user(email):
    try:
        uid = get_current_user_id(email)
        conn = get_connection(user_id=uid); cursor = conn.cursor()
        cursor.execute("DELETE FROM article_stats WHERE user_id = %s", (uid,))
        # app_usersはID申告なしのコネクションで削除
        conn_admin = get_connection(); cur_admin = conn_admin.cursor()
        cur_admin.execute("DELETE FROM app_users WHERE email = %s", (email,))
        conn_admin.commit(); conn_admin.close()
        conn.commit(); conn.close()
        return True
    except Exception: return False
...
def save_data(data, user_id=None):
    db_type, _ = get_db_info()
    try:
        conn = get_connection(user_id=user_id); cursor = conn.cursor()
        if db_type == "postgres":
...
def main():
    ...
    st.set_page_config(page_title="note分析 v6", layout="wide")
    ...
    if db_type == "postgres" and not st.session_state.app_auth_token:
        ...
        with tab_s:
            st.write("✨ **プレミアム note 分析ツール (v6 Cloud)**")
    ...
    if choice == "📥 データ管理":
        ...
        # データ取得状況の可視化
        try:
            conn = get_connection(user_id=uid)
...
2023-12-01,サンプル記事B,80,8,0"
    ...
    if st.sidebar.button("最新データを取得する"):
        ...
        if note_auth(s, ne, np):
            ...
            if data: 
                if save_data(data, user_id=uid): st.success("保存完了！"); st.rerun()
