...
- **UI挙動**: **アプリ独自のログイン画面やStripeの決済案内は一切表示されない。** 直接ダッシュボードが開く。
- **認証**: サイドバーのnote認証のみで動作。
- **用途**: 知識があるユーザーが自身のPCで個人利用する場合、または開発テスト用。

### 5.2 クラウド運用モード (Cloud Mode)
- **判定条件**: 環境変数 `DATABASE_URL` および `NEON_API_KEY` が設定済みの場合（Render等）。
- **データベース**: Neon (PostgreSQL) を使用。
- **収益化フロー**: 
    1. **アプリログイン**: 利用開始前にメールアドレスによる認証が必須。
    2. **新規登録・決済**: ユーザー自身でアカウント作成後、Stripe (月額300円) での決済が必要。
    3. **自動承認**: ログイン時に Stripe API で有効なサブスクリプションを確認し、自動的に利用を許可。
    4. **クーポン対応**: 決済時に `FREE30` (初月無料) や `SPECIAL200` (継続割引) を適用可能。
