...
- **認証方式**: 高信頼な「直接DB接続 (psycopg2)」によるSQL照合を採用。
    - パスワードは `pgcrypto` 拡張機能を用い、DB側で強力に暗号化 (`bf` アルゴリズム) して保存。
    - API経由の制限（JWTエンコーディング制限等）を回避し、高速かつ安定した認証を実現。
- **認証画面**: 
    1. **アプリログイン**: アプリ利用開始前にメールアドレスとパスワードによる認証が必須。
    2. **新規登録**: ユーザー自身によるアカウント作成が可能。
